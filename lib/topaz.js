...
// Tredelt ELLER. Én sann gjør hele svaret sant selv om resten er ukjent — en
// alarm er en alarm, og at vi mangler data fra naborommet gjør den ikke mindre
// ekte. Ingen sanne, men noe ukjent, gir ukjent: da vet vi ikke om det er
// rolig, og «rolig» er det eneste svaret som har lov til å stanse en flow.
...
// Full alarm per faretype for én enhet, pluss «any» som samlepost. Samme
// tredeling som ellers: null betyr at Nest ikke har sagt noe.
function alarms(state) {
  const smoke = isEmergency(state.smoke);
  const co = isEmergency(state.co);
  const heat = isEmergency(state.heat);

  return {
    smoke, co, heat, any: anyOf([smoke, co, heat]),
  };
}

// Samme vurdering på tvers av alle varslerne, til bruk i «en eller annen»-kort.
// En tom liste gir ukjent, ikke rolig: ingen enheter er mangel på data, ikke
// bevis på at det ikke brenner. Uten det ville et øyeblikk uten hentede bøtter
// — en omstart, en ny innlogging — sett ut som at alarmen ga seg.
function combinedAlarms(states) {
  const per = states.map((state) => alarms(state));
  const fold = (hazard) => anyOf(per.map((entry) => entry[hazard]));

  return {
    smoke: fold('smoke'), co: fold('co'), heat: fold('heat'), any: fold('any'),
  };
}
