...
class NestClient {
  ...
  async session() {
    ...
    const persistCookie = async (cookie) => {
      this._cookie = cookie;
      try {
        if (this._onCredentials) await this._onCredentials({ cookie });
      } catch (error) {
    ...
    const session = await authenticate(this._issueToken, this._cookie, { onCookie: persistCookie });
