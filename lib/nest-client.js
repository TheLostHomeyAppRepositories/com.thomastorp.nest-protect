...
class NestClient {
  ...
  async session() {
    ...
    this._log('Signing in to Nest');

    // Lagres i det øyeblikket Google roterer cookien, gjennom onCookie, ikke
    // etter at hele kjeden er ferdig. Feiler JWT- eller session-steget etterpå,
    // ville en sen lagring mistet rotasjonen, og neste fornyelse ville brukt den
    // gamle cookien — som Google til slutt svarer USER_LOGGED_OUT på.
    const persistCookie = async (cookie, { rotated = [] } = {}) => {
      this._cookie = cookie;
      try {
        if (this._onCredentials) await this._onCredentials({ cookie, rotated });
      } catch (error) {
        // En feilet skriving skal ikke velte innloggingen: verdien lever i
        // minnet denne økten uansett, og neste rotasjon prøver igjen.
        this._log('Could not save the rotated cookie', error && error.message);
      }
    };

    // Logges hver runde, også uten rotasjon. Slutter Google å rotere noen
    // runder før en USER_LOGGED_OUT, er det synlig i diagnoserapporten — og
    // det skiller «Google avsluttet økten» fra «vi mistet noe».
    const reportExchange = ({ changed, rotated, setCookieCount }) => {
      if (!changed) {
        this._log(`Token fetched, no cookie rotated (${setCookieCount} Set-Cookie from Google)`);
      }
    };

    const session = await authenticate(this._issueToken, this._cookie, {
      onCookie: persistCookie,
      onExchange: reportExchange,
    });
    ...
    this._log(`Signed in to ${session.transportUrl}`);
    return session;
  }
  ...
  async appLaunch({ bucketTypes = BUCKET_TYPES, timeoutMs = 30000 } = {}) {
    ...
    if (transportUrl && transportUrl !== this._session.transportUrl) {
      this._log(`Transport moved to ${transportUrl}`);
      this._session = { ...this._session, transportUrl };
    }

    this._log(`app_launch returned ${buckets.length} bucket(s)`);
    return buckets;
  }
